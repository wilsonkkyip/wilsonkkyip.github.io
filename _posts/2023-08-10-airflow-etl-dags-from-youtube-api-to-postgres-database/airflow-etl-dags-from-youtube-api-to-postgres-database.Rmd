---
title: "Airflow ETL Dags - From YouTube API to Postgres Database"
description: |
...
tags: [python, airflow, postgres, etl, youtube-api, etag, if-none-match]
categories:
  - python
  - airflow
  - postgres
...
abstract: |
  This article gives an introduction on how to write a dag on Airflow with an example of implementing an ETL pipeline from YouTube API to Postgres database. 
---
...
The subsections show what information can we obtain from `context` and `context.dag`.

### Context

As seen from [Airflow Github](https://github.com/apache/airflow/blob/main/airflow/utils/context.pyi), the `context` argument is of the `Context` class defined as below.
...
with DAG(
    ...
    etl_task >> [model_training_task1, model_training_task2, model_training_task3] >> model_selection_task
```
...
Go to Admin --> Variables, set up variables.
...
https://wilsonkkyip.github.io/posts/2023-08-05-introduction-to-docker-with-airflow-and-postgres-stack/


In [previous article](../2023-08-09-sample-youtube-channels-id-with-async-functions/), we have successfully obtained more than 300 channel ids. We will put them into 

https://wilsonkkyip.github.io/posts/2023-08-08-recursive-programming-querying-nested-json-in-python/

[previous article](../2023-08-08-recursive-programming-querying-nested-json-in-python/)

# YouTube Postgres ETL Pipeline
...
<div class="tab">
  <button id="london-btn" class="tablinks" onclick="openCity(event, 'London')">London</button>
...
document.getElementById("london-btn").click();
</script>
